...
class Fixed3StagePlanner(ExperimentPlanner):
    """
    Forces a 3-stage U-Net with fixed feature widths and number of blocks.
    ...
    def get_plans_for_configuration(self,
        ...
        # -----------------------------
        FIXED_NUM_STAGES = 3
        FIXED_FEATURES_PER_STAGE = (16, 32, 64)
        FIXED_BLOCKS_PER_STAGE = (1, 2, 2)
        FIXED_BLOCKS_DECODER = (2, 2)
        ...
        # ----------------------------------------------------
        # Infer pooling topology (truncate to 3 stages)
        # ----------------------------------------------------
        (
            _,
        ...
        if len(pool_op_kernel_sizes) < FIXED_NUM_STAGES:
            raise RuntimeError(
                ...
                f"but Fixed3StagePlanner requires 3."
            )
